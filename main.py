...
from util.constants import AUTH_USER, AUTH_PWD, save_path
import datetime
...
@app.post("/video/swap")
async def submit(video: video_store = Depends(), file: UploadFile = File(...)):
    ...
    # Create a new directory because it does not exist
        os.makedirs(save_path)
    #todo: check whether the video exist or not to re-download
